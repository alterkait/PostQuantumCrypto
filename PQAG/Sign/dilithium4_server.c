...
unsigned char sk[pqcrypto_sign_dilithium4_SECRETKEYBYTES];
unsigned char message[] = "This is an Authenticated Payload, Signed 7633254";
unsigned char signed_message[pqcrypto_sign_dilithium4_BYTES + sizeof(message)];
...
int main(){
        ...
        printf("Generating Dilithium4  key pair...\n");
        start_time = clock();
        pqcrypto_sign_dilithium4_keypair(pk, sk);
        end_time = clock();
        ...
        start_time = clock();
        pqcrypto_sign_dilithium4(signed_message, &signed_message_len, message, sizeof(message) - 1, sk);
        end_time = clock();
