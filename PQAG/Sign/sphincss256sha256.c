...
#include "pqcrypto_sign_sphincss256sha256.h"
...
unsigned char pk[pqcrypto_sign_sphincss256sha256_PUBLICKEYBYTES];
unsigned char sk[pqcrypto_sign_sphincss256sha256_SECRETKEYBYTES];
unsigned char message[] = "This is an Authenticated Payload, Signed 7633254";
unsigned char signed_message[pqcrypto_sign_sphincss256sha256_BYTES + sizeof(message)];
...
int main(){
        ...
        printf("Generating Sphincs key pair...\n");
        start_time = clock();
        pqcrypto_sign_sphincss256sha256_keypair(pk, sk);
        end_time = clock();
        ...
        start_time = clock();
        pqcrypto_sign_sphincss256sha256(signed_message, &signed_message_len, message, sizeof(message) - 1, sk);
        end_time = clock();
